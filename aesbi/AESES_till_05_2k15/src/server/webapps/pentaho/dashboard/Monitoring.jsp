...
String query="select monitoring_user, monitoring_pass from APPLICATIONS where pentaho_user='"+user.getUsername()+"'";
int i=1;
con=DriverManager.getConnection(url);
stmt=con.createStatement();
rst=stmt.executeQuery(query);
while(rst.next()){
request.setAttribute("__ac_name",rst.getString(1));
String decryptedPassword = e.decrypt(rst.getString(2));
request.setAttribute("__ac_password",decryptedPassword);
}}
rst.close();
stmt.close();
con.close();
...
<form  action="http://enst01nm01pr.aesenergystorage.com:8080/" name="logonForm2" method="post"  target="_self">
  <input name="__ac_name" id="__ac_name" value="<%=request.getAttribute("__ac_name")%>" type="hidden" />
  <input name="__ac_password" id="__ac_password" type="hidden" value="<%=request.getAttribute("__ac_password")%>"/>
 </form>
</body>
</html>